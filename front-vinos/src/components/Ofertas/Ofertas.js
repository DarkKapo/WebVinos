...
export default function OfertasPag ({ agregarCarro }) {
    localStorage.removeItem('filtroBusqueda')
    //Estilo de la carta ques e muestra en cada producto
    const styles = {
        producto: {
    ...
    const obtenerProductos = async () => {
        const { data } = await axios.get('http://localhost:8080/ofertas')
